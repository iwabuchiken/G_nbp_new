...
void close_file( FILE *fp, char *fname );
int validate_file_name(char *fname, const char *str1); // 2011/11/10-08:15:41
char * get_fname4( char *fname_in, char *ext, char *memo ); // 2011/11/10-09:53:25
void d2b(int bin, char *bnum); // 2011/11/14-15:03:34
int b2d(char *bnum); // 2011/11/14-15:03:34

// func defs =============================
int b2d(char *bnum)
{
    int blen; // length of bnum
    int i; // counter
    int sum = 0;
    int digit; // judge
    ...
    for (i = 0; i < blen; i++) {
        ...
        sum += digit * pow(2, (blen - i - 1));
        //sum += atoi(bnum[i] * pow(2, (blen - i - 1)));
//        sum += atoi(bnum[i]) * pow(2, (blen - i - 1)));
    }//for (i = 0; i < blen; i++) {
...
void d2b(int bin, char *bnum)
{
    int     i, j; // i: param, j: counter
    //char *buff[MAX_BUFF];
    char buff[MAX_BUFF];
    ...
    for(i = 0; i < MAX_BUFF - 1; i++){
            ...
            if(bin % 2 == 0)
                    buff[(MAX_BUFF - 1 - 1) - i] = '0';
            else
                    buff[(MAX_BUFF - 1 - 1) - i] = '1';
            bin = bin / 2;
    }

    printf("[%s:%d]\n", __FILE__, __LINE__);
    printf(">>>buff+(MAX_BUFF-1)-i: %s(%d)\n",
            buff+(MAX_BUFF-1)-i, strlen(buff+(MAX_BUFF-1)-i));

    for (j = 0; j < strlen(buff+(MAX_BUFF-1)-i); j++) {
        bnum[j] = buff[(MAX_BUFF - 1) - i + j];
    }//for (j = 0; j < strlen(buff+(MAX_BUFF-1)-i); j++) {
    bnum[j] = '\0';
    printf("[%s:%d]\n", __FILE__, __LINE__);
    printf("bnum=%s(%d)\n", bnum, strlen(bnum));
...
int validate_file_name(char *fname, const char *str1)
{
    // 1: valid -> file name has an extension str1
    // 0: invalid -> file name doesn't have an extension str1
    // -1: error -> doesn't have an extension
    char *file_name; // receive fname
    char *s1, *s2; // tokens

    // malloc
    file_name = malloc(sizeof(strlen(fname)));
    // strcpy
    ...
    free(file_name);
    return -1;
}//int validate_file_name(char *fname, char *str1)
...
int get_basename2(
            char *file_name, int size, char *basename, int n)
{
    // return -1: file name doesn't have '.'
    // int n: number
...
int get_basename(
            char *file_name, int size, char *basename)
{
    // return -1: file name has no '.'
    // return k: the position number of '.'
...
//char * get_fname3( char *fname_in, char *ext, char **memo )
char * get_fname4( char *fname_in, char *ext, char *memo )
{
    char *time_label = NULL;
    char *fname_out = calloc(
                strlen(fname_in)
    ...
    time_label = get_time_label();
    sprintf(fname_out, "%s_%s_%s.%s",
                fname_in, memo, time_label, ext);
                //fname_in, time_label, memo, ext);
    free(time_label);
