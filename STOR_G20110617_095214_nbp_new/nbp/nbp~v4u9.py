...
VERSION = ["4.9", "check_isdir(target_dir)"]
...
#	f_src = "%s%s%s" % os.path.join(dir_src, group_dir, fname)

	#debug
#	print f_src; sys.exit()

#	f_src = "%s%smain.cpp" % (dir_src, os.sep)
...
#	# create 'cpp' dir
#	target_dir = os.path.join(directory, "G" + time_label, \
#			group_dir)
#	if not os.path.isdir(target_dir):
#		try:
#			os.mkdir(target_dir)
#			print "New dir created: %s" % target_dir
#		except Exception, e: print e
...
def create_dirs(directory, time_label, command1, command2):
	try:
		os.system(command1)
...
#  time_label = get_time_label2()
  ...
  create_dirs(directory, time_label, command1, command2)
...
#  create_main_js(dir_src, directory, time_label)
  ...
  # cpp project
  if proj == 'cpp': create_cpp_files(directory, dir_src, time_label)
	  # main.cpp
#	  create_main_cpp(directory, dir_src, time_label)
