...
#                      get_time_label3())
	...
	word_list = {
			...
			"{{debug}}": 'print "[DEBUG:%d]" % inspect.currentframe().f_lineno; sys.exit()'
	}
...
def _create_commands_txt(directory, dir_src, time_label, group_dir, ext='py'):
#	fname = "%s" % f_name
	fname = "commands.txt"
	f_src = "%s" % os.path.join(dir_src, group_dir, fname)
...
#	while (True):
	...
	try:
		os.system(command1)
		print "Directory created: %s" % os.path.join(directory, "PIC10_" + time_label)
...
#	if not proj == "ee": create_dirs_ee(directory, time_label, yes_flag)
	...
	elif proj == 'ee': create_ee_files(directory, dir_src, time_label)
