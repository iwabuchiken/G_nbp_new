...
#								if re.search(reg, item)]
		...
		reg = re.compile("^(%s)~v([\d]+)u([\d]+)\.(%s)"
									% (name.split(".")[0], name.split(".")[1]))
#		a_reg = [re.search(reg, item).groups() for item in items]
		a_reg = [re.search(reg, item).groups() for item in items if re.search(reg, item)]
#		a_reg = [reg.search(reg, item).groups() for item in items]
		print "a_reg=%s" % a_reg
		print
...
#		a_reg.sort(cmp=lambda x,y:cmp(x[2], y[2]))
#		a_reg.sort(cmp=lambda x,y:cmp(int(x[2]), int(y[2])))
#		a_reg.sort(key=lambda x: int(x[2]))
#		a_reg_int.sort(key=lambda x: int(x[2]))
#		a_reg_int.sort(key=lambda x: (int(x[1]), int(x[2])))
		a_reg_int.sort(key=lambda x: (int(x[1]), int(x[2])), reverse=True)
#		print "a_reg_int=%s" % a_reg_int
#		print "a_reg_int[0]=%s" % a_reg_int[0]
...
#	sys.exit(0)
	...
	for item in final_jar_files:
		if type(item) == tuple:
			...
			final_jar_files2.append(item)

#	final_jar_files = [list(item) for item in final_jar_files]
	print "[DEBUG:%d]" % inspect.currentframe().f_lineno; #sys.exit()
	print "final_jar_files=%s" % final_jar_files
